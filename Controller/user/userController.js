...
async function updateProfile(req, res) {
    ...
    try {
        ...
        if (username) {
            const [usernameExists] = await dbConnection.query("SELECT * FROM users WHERE username = ? AND userid != ?", [username, userId]);
        ...
        if (email) {
            const [emailExists] = await dbConnection.query("SELECT * FROM users WHERE email = ? AND userid != ?", [email, userId]);
        ...
        if (currentPassword || newPassword || retypeNewPassword) {
            ...
            const [user] = await dbConnection.query("SELECT password FROM users WHERE userid = ?", [userId]);
